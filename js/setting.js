...
function SETTING(setting_data, booking_package_dictionary, webApp) {
	...
	**/
	...
	this.loadTabFrame = function(){
		...
		var menuList = {settingLink: 'settingPanel', holidayLink: 'holidayPanel', nationalHolidayLink: 'nationalHolidayPanel', memberLink: "memberPanel", /**formLink: 'formPanel', courseLink: 'coursePanel', emailLink: 'emailPanel',**/ syncLink: "syncPanel", cssLink: "cssPanel", subscriptionLink: 'subscriptionPanel'};
	...
	this.syncPanel = function(){
		...
		var google_calendar_api_panel = function(messagePanel, parse_url, bookingSyncList){
    		...
    		console.log(bookingSyncList.booking_package_googleCalendar_json);
		...
		console.log(object._bookingSyncList);
    	...
    	for(var i in object._bookingSyncList){
    		...
    		for(var key in object._bookingSyncList[i]){
    			...
    			console.log(object._bookingSyncList[i]);
    	...
    	if(inputData.booking_app_googleCalendar_json){
    		...
    		console.log(inputData.booking_app_googleCalendar_json.textBox);
    		...
    		if(value.length != 0){
    			...
    			console.log(client_id_for_google);
        	...
        	for(var i in object._bookingSyncList){
        		...
        		for(var key in object._bookingSyncList[i]){
					...
					if(inputData[key] != null){
						...
						console.log(key);
						console.log(object._bookingSyncList[i][key]);
	...
	this.holidayPanel = function(mode){
        ...
        var calendar = new Booking_App_Calendar(weekName, object._setting_data.dateFormat, object._setting_data.positionOfWeek, object._startOfWeek, object._i18n);
        ...
        calendar.create(calendarPanel, regularHolidays, month, 1, year, '', function(callback){
        	...
        	callback.eventPanel.onclick = function(){
        		...
        		var holiday = regularHolidays.calendar[key];
        		console.log(key);
        		console.log(regularHolidays.calendar);
        		console.log(holiday);
	...
	this.memberPanel = function(){
		...
		for(var key in memberSetting){
			...
			if(inputElement.tagName == "INPUT" && inputElement.type.toLocaleUpperCase() == "CHECKBOX"){
				...
				console.log(inputElement.type.toLocaleUpperCase());
	...
	this.createSettingPanel = function() {
		...
		console.log(object._settingList);
    	...
    	for(var i in object._settingList){
    		...
    		var disabled = false;
    		if(i == 'Mailgun' || i == 'Stripe' || i == 'PayPal'){
    			/**
    			if(typeof ExtensionsFunction != "function"){
        	...
        	for(var i in object._settingList){
        		...
        		for(var key in object._settingList[i]){
					...
					if(inputData[key] != null){
						...
						console.log(key);
						console.log(object._settingList[i][key]);
						console.log(inputData[key]);
	...
	this.createInput = function(inputName, input, inputData, disabled, idBool){
		...
		if(input['inputType'] == 'TEXT'){
			...
			console.log("inputType = " + input['inputType']);
			console.log(input);
			var options = [];
			if(input['value'] != null){
		...
		function create_tr(tr_index, table, value){
			...
			textBox.onchange = function(){
				...
				console.log("dataKey = " + dataKey + " value = " + value + " bool = " + bool);
