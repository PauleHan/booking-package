<?php
    ...
    class booking_package_schedule {
					...
					if($bool === true){
						...
						if(!is_int($value) && is_array(array_values($value))){
							...
							**/
							...
							$response = array("status" => 0, "user" => array_merge($memberSetting, array("status" => 0, "message" => __('', $pluginName), "reload" => 1)));
							
						}
						
					}else{
						
						$response = array("status" => 0, "user" => array_merge($memberSetting, array("status" => 0, "message" => __('', $this->pluginName), "reload" => 1)));
    	...
    	public function updateAccountSchedule(){
    		...
    		$row = $wpdb->get_row($wpdb->prepare($sql, array(intval($accountKey))), ARRAY_A);
    		if(is_null($row)){
    	...
    	public function getReservationData($month, $day, $year, $ical = false, $public = false){
			...
			if($ical === false){
				...
				$arrayValue = array('startDay' => 1, 'lastDay' => $last_day, 'startWeek' => $week_start_num, 'lastWeek' => $week_last_num, 'year' => $year, 'month' => intval($month), 'day' => 1, 'timestamp' => date('U'), 'today' => intval(date('Ymd')), 'firstMonth' => intval(date('U', mktime(0, 0, 0, $month, 1, $year))), 'endMonth' => intval(date('U', mktime(23, 59, 59, $month, $last_day, $year))));
				...
				if($public !== false && $changeMonth === true /**$month == date('n')**/){
					...
					$schedule_start_day = intval(date('Ymd', mktime(0, 0, 0, $month, $day, $year)));
						...
						if($public == false){
							...
							if(is_null($rows) === false && count($rows) != 0){
								...
								for($row = 0; $row < count($rows); $row++){
									...
									/**
									if($account['type'] == 'hotel' && $visitorList[$rows[$row]['key']] > 1 && intval($calendarUnixTime) != intval($rows[$row]['checkIn'])){
    	...
    	public function getDownloadCSV(){
			...
			foreach($rows as $row){
				...
				if($calendarAccount['type'] == 'day'){
					...
					$visitor['scheduleDate'] = $this->dateFormat($dateFormat, $positionOfWeek, $row['scheduleUnixTime'], $row['title'], true, false);
    	...
    	public function createAccommodationDetails($calendarAccount, $json, $sql_start_unixTime, $applicantCount, $accommodationDetails = null){
            			...
            			**/
            			...
            			if($i != 0){
            				...
            				if($time < 1440){
            					...
            					return array("status" => "error", "message" => __("Schedule data was not found"), "sql" => $sql);
    		...
    		/**	
    			...
    			return array("status" => "error", "message" => __("Schedule data was not found"));
    	...
    	public function createTaxesDetailes($accountKey, $totalCost) {
    	...
    	public function sendBooking($administrator = false) {
    		...
    		if(is_null($row)){
    			...
    			if(!is_null($holiday) && intval($holiday['status']) == 1){
    				...
    				#$wpdb->query('ROLLBACK');
    				return array("status" => "error", "message" => __("Schedule data was not found"), 'reload' => 1);
				...
				if($row['unixTime'] == $scheduleUnixTime){
					...
					if($calendarAccount['type'] == "hotel" && isset($_POST['json'])){
    					...
    					#createAccommodationDetails($calendarAccount, $json, $sql_start_unixTime, $applicantCount, $accommodationDetails = null)
    					$accommodationDetails = $this->createAccommodationDetails($calendarAccount, $_POST['json'], $sql_start_unixTime, $applicantCount, null);
						...
						$taxes = $this->createTaxesDetailes($accountKey, $totalCost);
    		...
    		if(intval($_POST['sendEmail']) == 1){
    			...
    			$email = $this->createEmailMessage($accountKey, array('mail_new_admin'), $form, $accommodationDetails, $selectedOptions, $lastID, $scheduleUnixTime, $cancellationUri, $currency, $services, $scheduleTitle);
    	...
    	public function cancelBookingData($deleteKey, $token) {
    		...
    		if ($verifyCancellation['cancel'] === true) {
    			...
    			if ($deleteResponse['status'] == 'success') {
    				...
    				$service = null;
    				if (strlen($myBookingDetails['courseName']) > 0) {
    	...
    	public function deleteBookingData($deleteKey = false, $sendGC = true, $deleteVisitorDetails = true, $sendEmail = 1){
    		...
    		if($deleteKey !== false){
            	...
            	if(is_null($row) === false){
    				...
    				$mailBody = null;
    				#if(intval(get_option($this->prefix."mail_deleted_enable", null)) == 1 && intval($_POST['sendEmail']) == 1){
    				if (intval($sendEmail) == 1) {
    					
    					$email = $this->createEmailMessage($accountKey, array('mail_deleted'), $form, $accommodationDetails, $options, $deleteKey, intval($unixTimeStart), null, $currency, $services, $scheduleTitle);
    					$ressponse['email'] = $email;
            ...
            if(is_null($row) === false){
				...
				**/
				...
				if($calendarAccount['type'] == 'hotel'){
					...
					#createAccommodationDetails($calendarAccount, $json, $sql_start_unixTime, $applicantCount, $accommodationDetails = null)
    				$accommodationDetails = json_decode($row['accommodationDetails'], true);
    				$accommodationDetails = $this->createAccommodationDetails($calendarAccount, $_POST['json'], $unixTimeStart, $applicantCount, $accommodationDetails);
    				if($accommodationDetails['status'] == "error"){
    	...
    	public function updateStatus(){
    		...
    		if(intval($sendEmail) == 1){
    			...
    			$email = $this->createEmailMessage($accountKey, array($email_id), $form, $accommodationDetails, $options, intval($_POST['key']), intval($unixTimeStart), null, $currency, $services, $scheduleTitle);
    			$ressponse['email'] = $email;
    			#return $email;
    		}
    	...
    	public function createEmailMessage($accountKey, $email_id, $form, $accommodationDetails, $options, $bookingID, $unixTime, $cancellationUri, $currency = 'usd', $services = null, $scheduleTitle = null){
