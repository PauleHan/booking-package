	...
	class BOOKING_PACKAGE {
		...
		public function __construct() {
			...
			**/
			...
			add_action('wp_before_admin_bar_render', array($this, 'adomin_toolbar'));
		...
		public function adomin_toolbar(){
		...
		public function add_pages() {
			...
			$setting = new booking_package_setting($this->prefix, $this->plugin_name);
			/**
			$response = $setting->updateChannelGC($calendarAccountList);
			if(!is_null($response) && !isset($response['status']) && is_bool($response) === false && is_array($response) === true){
				
				for($i = 0; $i < count($response); $i++){
					
					$webhookDetails = $response[$i];
					$schedule->updateCalendarAccountForGoogleWebhook($webhookDetails['key'], $webhookDetails['id'], $webhookDetails['expiration']);
			...
			foreach($lists as $key => $value){
				...
				if(isset($calendarAccountList[$key])){
					...
					for($i = 0; $i < count($value); $i++){
						...
						$this->showVisitorsList($value[$i], $url, $calendarAccountList[$key]['type'], $schedule, $dateFormat);
		...
		public function showVisitorsList($visitor, $url, $calendarType, $schedule, $dateFormat){
		...
		public function bookingPageForVisitors($atts){
			...
			#var_dump($atts);
			$atts = extract(shortcode_atts(array('id' => 1), $atts, "booking_package"));
			$accountKey = $id;
			...
			unset($calendarAccount['googleCalendarID']);
			unset($calendarAccount['idForGoogleWebhook']);
			unset($calendarAccount['expirationForGoogleWebhook']);
			...
			**/
			...
			if (isset($_GET[$this->prefix.'login_error'])) {
				...
				$html .= "<div class='login_error'>" . $_GET[$this->prefix.'login_error'] . "</div>";
			...
			$html .= '<div id=""></div>';
			$html .= '	<div id="booking-package_calendarPage"></div>';
			$html .= '	<div id="booking-package_durationStay"></div>';
					...
					<div class="edit_modal_content">
						...
						<div id="media_router" class="media_frame_router">
							<div class="reservation_table_row">
								...
								<div id="add_reservation" class="media_menu_item"><?php _e("Book now", $pluginName); ?></div>
							</div>
						</div>
		...
		public function schedule_page(){
			...
			wp_enqueue_script('Calendar_js', plugin_dir_url( __FILE__ ).'js/Calendar.js'.$p_v);
			wp_enqueue_script('input_js', plugin_dir_url( __FILE__ ).'js/Input.js'.$p_v);
			wp_enqueue_script('schedule_pange', plugin_dir_url( __FILE__ ).'js/Schedule.js'.$p_v, array('jquery', 'wp-color-picker'));
					...
					<div class="edit_modal_content">
						...
						<div id="edit_schedule_for_hotel" class="media_left_zero hidden_panel">
							...
							<div class="enablePanel">
								<div class="enableLabel"><?php _e("Booking acceptance", $pluginName); ?></div>
								<div class="enableValuePanel">
									<label><input type="checkbox" id="bookingEnable"/><?php _e("Enable", $pluginName); ?></label>
								</div>
							</div>
							-->
						...
						<div id="email_edit_panel" class="media_left_zero hidden_panel">
							...
							<div class="mail_message_area_left">
								...
								<div id="emailContentPanel" class="text_editer_panel">
			                	...
			                	<tr>
			                		...
			                		<td>
			                			...
			                			<?php
			                				...
			                				for ($i = 0; $i < 700; $i += 30) { echo '<option value="'.$i.'">' . sprintf(__('%s min ago', $this->plugin_name), $i) . '</option>'; }
			                ...
			                <div id="selectionSchedule_deadline" class="selectBlock">
			                	...
			                	<div data-key="deadline" class="selectPanel closed">
			                		...
			                		<?php
			                			...
			                			for ($i = 0; $i <= 720; $i += 30) { echo '<span class="selectItem" data-key="deadline" data-value="' . $i . '">' . sprintf('%02d', $i) . '</span>'; }
		...
		public function setting_page(){
			...
			wp_enqueue_script('Confirm_js', plugin_dir_url( __FILE__ ).'js/Confirm.js'.$p_v);
			wp_enqueue_script('Calendar_js', plugin_dir_url( __FILE__ ).'js/Calendar.js'.$p_v);
			wp_enqueue_script('setting_page', plugin_dir_url( __FILE__ ).'js/setting.js'.$p_v, array('jquery', 'wp-color-picker'));
					...
					<div id="tabFrame">
						...
						<div id="contentPanel" class="content">
							...
							<div id="cssPanel" class="hidden_panel">
								...
								<div><button id="save_css" type="button" class="w3tc-button-save button-primary" style="margin-top: 1em;"><?php _e("Save CSS", $pluginName); ?></button></div>
							</div>
		...
		**/
		...
		public function getDictionary($mode, $pluginName){
			...
			if($mode == "adomin"){
				...
				$dictionary['Please create a service on the setting page.'] = __('Please create a service on the setting page.', $pluginName);
				$dictionary['There is no room to booking'] = __('There is no room to booking', $pluginName);
				...
				$dictionary['Deadline time'] = __('Deadline time', $pluginName);
				$dictionary['%s min age'] = __('%s min age', $pluginName);
				...
				$dictionary['Plan name'] = __('Plan name', $pluginName);
				...
				$dictionary['Add filed'] = __('Add filed', $pluginName);
				...
				$dictionary['Google Calendar ID'] = __('Google Calendar ID', $pluginName);
				...
				$dictionary['Type'] = __('Type', $pluginName);
				$dictionary['Options'] = __('Options', $pluginName);
				#$dictionary['New booking(Admin)'] = __('New booking(Admin)', $pluginName);
				$dictionary['Email is sending to Admin after Making of booking.'] = __('Email is sending to Admin after Making of booking.', $pluginName);
				#$dictionary['New booking(Visitor)'] = __('New booking(Visitor)', $pluginName);
				$dictionary['Email is sending to visitor after Making of booking.'] = __('Email is sending to visitor after Making of booking.', $pluginName);
				#$dictionary['Approval of booking'] = __('Approval of booking', $pluginName);
				#$dictionary['Pending of booking'] = __('Pending of booking', $pluginName);
				$dictionary['Booking status is set to Pending'] = __('Booking status is set to Pending', $pluginName);
				$dictionary['Email is sending to Visitor after Booking set as Pending.'] = __('Email is sending to Visitor after Booking set as Pending.', $pluginName);
				$dictionary['Cancellation of booking'] = __('Cancellation of booking', $pluginName);
				...
				#$dictionary['By using iCal you can share calendars and reservations for Google Calendar, Apple Calendar, iCal format.'] = __('By using iCal you can share calendars and reservations for Google Calendar, Apple Calendar, iCal format.', $pluginName);
				#$dictionary['By using the Google Calendar API you can share information on reserved users with Google Calendar. And you can make and edit new booking from Google Calendar.'] = __('By using the Google Calendar API you can share information on reserved users with Google Calendar. And you can make and edit new booking from Google Calendar.', $pluginName);
				#$dictionary['Mailgun is an email automation service. It offers a complete cloud-based email service for sending, receiving and tracking email sent through your websites and applications.'] = __('Mailgun is an email automation service. It offers a complete cloud-based email service for sending, receiving and tracking email sent through your websites and applications.', $pluginName);
				$dictionary['Stripe is the best way to accept payments online and in mobile apps.'] = __('Stripe is the best way to accept payments online and in mobile apps.', $pluginName);
				...
				$dictionary['Reservation Completed'] = __('Reservation Completed', $pluginName);
